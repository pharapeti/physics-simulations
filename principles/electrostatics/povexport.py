...
# John Denker
# 2015-09-04
# Remove confusion between "display" as a local variable and its definition as a VPython class,
# and fix a bug in which X was wrong in X.mouse.pos.
...
def export(display=None, filename=None, include_list=None, xy_ratio=4./3.,
                 custom_text='', shadowless=0):

    # The interface (above) uses the documented variable name "display".
    # Internally we use the name "scene" which is better for coding, since
    # it doesn't conflict with the name of the class of which it is an instance.
    scene = display
    del display
    # At this point, anybody who needed the class itself could get it
    # easily enough, but we don't actually need it:
    # display = type(scene)     # (not needed)

    global displayscale
    if scene == None:         # no display specified so find active display
        b = box(visible=0)
        scene = b.display
        del b

    if filename == None:
        filename = scene.title + '.pov'
...
"""
    ...
    if type(scene.ambient) is tuple or type(scene.ambient) is list:
        file.write( ambient_template % { 'red':scene.ambient[0]*10 ,
                                         'green':scene.ambient[1]*10,
                                         'blue':scene.ambient[2]*10 })
    else:
        file.write( scalar_ambient_template % { 'amb':scene.ambient*10 } )
    file.write( background_template % { 'red':scene.background[0],
                                        'green':scene.background[1],
                                        'blue':scene.background[2] } )

    displayscale = 10.0/(mag(scene.mouse.camera-scene.center)*tan(scene.fov/2.))

    for light in scene.lights: # reproduce visual lighting (not ideal, but good approximation)
        try:
    ...
    cpos = displayscale*scene.mouse.camera
    ctr = displayscale*scene.center
    cup = scene.up
